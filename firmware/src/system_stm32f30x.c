...
#define HSE_STARTUP_TIMEOUT  ((uint16_t)0xA000)   /*!< Time out for HSE start up */
...
static void SetSysClock(void)
{
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
	...
	//new configuration
	//13.56MHz crystal
	//PLLCLK = HSE * 5 = 13.56*5 = 67.8MHz
    RCC->CFGR &= (uint32_t)((uint32_t)~(RCC_CFGR_PLLSRC | RCC_CFGR_PLLXTPRE |
                                        RCC_CFGR_PLLMULL));
    RCC->CFGR |= (uint32_t)(RCC_CFGR_PLLSRC_PREDIV1 | RCC_CFGR_PLLMULL10 | RCC_CFGR_PLLXTPRE_PREDIV1_Div2);
  }
  ...
  { /* If HSE fails to start-up, the application will have wrong clock 
	...
	//this should setup the other clocks
  
    /* Enable Prefetch Buffer and set Flash Latency */
    FLASH->ACR = FLASH_ACR_PRFTBE | FLASH_ACR_LATENCY_1;

	//do not divide the system clock
    /* HCLK = SYSCLK */
    RCC->CFGR |= (uint32_t)RCC_CFGR_HPRE_DIV1;
      
	
    /* PCLK2 = HCLK */
	//PCLK2   - AHBCLK / APB2 prescaler ; max 72MHz, no divide
    RCC->CFGR |= (uint32_t)RCC_CFGR_PPRE2_DIV1;
    
    /* PCLK1 = HCLK */
	//PCLK1   - AHBCLK / APB1 prescaler ; max 36MHz, hence the divide by two
